	...
	private function _minify($flag, $file_ref)
	{
		...
		switch($flag){
			...
			case 'js':
				...
				if ($this->minify_driver)
				{
				    $this->_load('minify');
    				return $this->CI->minify->js->min($contents);
				}
				else 
				{
				    $this->_load('jsmin');
			...
			case 'css':
                ...
                {
                    $this->_load('minify');
                    $this->CI->minify->css->config(array('relativePath'=>$rel));
                    $contents = $this->_get_contents( $ref );
                	return $this->CI->minify->css->minify($contents);
                }
                else 
                {
                    $this->_load('cssmin');
